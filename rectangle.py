...
class Rectangle:
    ...
    def perimeter(self):
        return 2 * ((self._xint.upper() - self._xint.lower()) + (self._yint.upper() - self._yint.lower()))

    # Returns True if this rectangle contains the point (x, y), and False otherwise.
    def contains(self, x, y):
        if self._xint.contains(x) == True and self._yint.contains(y) == True:
            return True
        return False

    # Returns True if this rectangle intersects other, and False otherwise.
    def intersects(self, other):
        if self._xint.intersects(other._xint) == True and self._yint.intersects(other._yint) == True:
            return True
        return False
